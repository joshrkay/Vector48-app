  ...
  try {
    ...
    for (const activation of activations ?? []) {
      ...
      const pausedIds = Array.isArray(cfg.paused_contact_ids)
        ? (cfg.paused_contact_ids as string[])
        : [];

      // Idempotent: only update if not already paused
      if (!pausedIds.includes(contactId)) {
        await admin
          .from("recipe_activations")
          .update({ config: { ...cfg, paused_contact_ids: [...pausedIds, contactId] } })
          .eq("id", activation.id);
      }
