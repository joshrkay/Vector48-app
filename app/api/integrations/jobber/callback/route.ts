...
export async function GET(req: Request) {
  ...
  if (!clientId || !clientSecret) {
    return redirectWithError("jobber_oauth_not_configured");
  }
  ...
  if (!tokenRes.ok) {
    console.error("[jobber callback] token exchange", tokenRes.status, tokenJson);
  ...
  if (row) {
    const { error: updateErr } = await admin
      ...
      .eq("id", row.id);
    if (updateErr) {
      console.error("[jobber callback] integration update", updateErr.message);
    ...
    if (insertErr) {
      console.error("[jobber callback] integration insert", insertErr.message);
      return redirectWithError("integration_save_failed");
    }
  }
