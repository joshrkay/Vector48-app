...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (account.ghl_provisioning_status !== "failed") {
      return respond({ error: "Provisioning is not in a retryable state" }, 409);
    }
    ...
    if (updateError) {
      console.error("[api/onboarding/provision/retry] failed to retry provisioning", {
        accountId,
        requestId,
        status: 500,
        error: updateError,
      });
      return respond({ error: "Failed to retry provisioning" }, 500);
    }
    ...
    try {
      ...
      console.error(
        "[api/onboarding/provision/retry] failed to enqueue provisioning job",
        {
          accountId,
          requestId,
          status: 500,
    ...
    console.error("[api/onboarding/provision/retry] unexpected error", {
      requestId,
      status: 500,
      error,
    });
